...
import { useGetArticlesQuery, useGetReadersQuery, useGetReactionsQuery } from "./api/articleApi";
...
const menus = [
  ...
  { label: "Help", items: [{ label: "Beta Reading Guide" }, { label: "Keyboard Shortcuts" }] },
];
...
export default function App() {
  ...
  const { data: readers = [] } = useGetReadersQuery(selectedArticle?.id || articles[0]?.id || "", { skip: articles.length === 0 });
  const { data: reactions = [] } = useGetReactionsQuery(selectedArticle?.id || articles[0]?.id || "", { skip: articles.length === 0 });
